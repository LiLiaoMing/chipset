...
// bool GetOneLine(char* buf, int buflen, const char* pSrcBuf)
// {
// 	int k, len = 0;
// 	const char *lpSrc = pSrcBuf;
// 
// 	for( k=0;k<buflen;k++) {
// 		if(lpSrc[k]==0x0d || lpSrc[k]==0x0a)
// 			break;
// 		buf[k] = lpSrc[k];
// 		len ++;
// 	}
...
// 		return false;
// 
// 	if(lpSrc[k+1]==0x0d || lpSrc[k+1]==0x0a)
// 		pSrcBuf += (k+1);
// 	else
// 		pSrcBuf += (k);
// 
...
bool GetOneLineEx(CString& strLine, CStringArray& pSrcBuf)
{
	...
	if(pSrcBuf.IsEmpty()) return false;

	strLine = pSrcBuf.GetAt(0);
...
bool GetOneLine(char* buf, int buflen, CStringArray& pSrcBuf)
{
	

	if(pSrcBuf.IsEmpty()) return false;

	CString strOneLine = pSrcBuf.GetAt(0);

	ZeroMemory(buf, buflen);
	memcpy(buf, (LPCTSTR)strOneLine, strOneLine.GetLength());
...
int handleCommands(CStringArray& pCmdLines)
{
	...
	TCHAR *token;
	//const char  *pSrc = pCmdLines;
	OptionStr optionStr;
	OPGP_ERROR_CREATE_NO_ERROR(status);

	memset(&optionStr, 0, sizeof(optionStr));
	//while (_fgetts (buf, BUFLEN, fd) != NULL)
	//while (GetOneLine (buf, BUFLEN, pSrc))
	while (GetOneLine (buf, BUFLEN, pCmdLines))
	{
...
#define NUM_APPLICATIONS 64
			...
			else if (_tcscmp(token, _T("send_apdu")) == 0 || _tcscmp(token, _T("send_apdu_nostop")) == 0)
			{
				...
				memset(g_recvAPDU, 0, sizeof(g_recvAPDU));
				g_recvAPDULen = 0;
				...
				g_recvAPDULen = recvAPDULen;
				memcpy(g_recvAPDU, recvAPDU, recvAPDULen);
